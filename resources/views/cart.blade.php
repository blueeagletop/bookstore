...
@section('content')
  <div class="page bk_content" style="top: 0;">
    <div class="weui_cells weui_cells_checkbox">
        ...
        <label class="weui_cell weui_check_label" for="{{$cart_item->book->id}}">
            <div class="weui_cell_hd" style="width: 23px;">
                <input type="checkbox" class="weui_check" name="cart_item" id="{{$cart_item->book->id}}" checked="checked">
                <i class="weui_icon_checked"></i>
            </div>
            <div class="weui_cell_bd weui_cell_primary">
              <div style="position: relative;">
                <img class="bk_preview" src="http://localhost/laravel52/public{{$cart_item->book->preview}}" class="m3_preview" onclick="_toBook({{$cart_item->book->id}});"/>
...
<form action="order_commit" id="order_commit" method="post">
    {{ csrf_field() }}
    <input type="hide" name="book_ids" value="" />
...
//      location.href = 'order_pay?cart_item_ids=' + cart_item_arr;
    ...
    $('input:checkbox[name=cart_item]').each(function(index, el) {
      if($(this).attr('checked') == 'checked') {
        book_ids_arr.push($(this).attr('id'));
      }
    });

    if(book_ids_arr.length == 0) {
      $('.bk_toptips').show();
...
//        location.href = 'order_commit/' + book_ids_arr;
//    location.href = '/order_commit?book_ids=' + book_ids_arr + '&is_wx=' + is_wx;
     $('input[name=book_ids]').val(book_ids_arr+'');
